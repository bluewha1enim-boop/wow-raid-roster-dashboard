...
async function apiRequest(path, options = {}) {
  const target = toApiRequest(path, options.method || "GET");
...
function toApiRequest(path, method) {
  const scheduleMatch = path.match(/^\/api\/schedules\/([^/?#]+)$/);
  if (scheduleMatch) {
    const id = encodeURIComponent(scheduleMatch[1]);
    if (method === "PUT") {
      return { url: `api.php?action=schedule-update&id=${id}`, method: "POST" };
    }
    if (method === "DELETE") {
      return { url: `api.php?action=schedule-delete&id=${id}`, method: "POST" };
    }
    return { url: `api.php?action=schedule&id=${id}`, method };
  }
  ...
  if (routes[path]) {
    return { url: `api.php?action=${routes[path]}`, method };
  }
